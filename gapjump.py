...
def find_university_info(soup):
    """Find university info in parsed html text"""
    # need to fine tune this regex
    pattern = "([A-Z][^\s,.]+[.]?\s[(]?)*(College|University|University of|UC|Institute|Law School|School of|Academy)[^,\d]*(?=,|\d)"
    university = None
    try:
        #linkedin headline if any
        desc2 = soup.find('div', attrs={'class': 'f', 'class': 'slp'}).get_text()
...
# open the csv file, parse it and start running through each line(name and email)
with open(csv_file) as csvfile1:
    ...
    for row in reader:
        ...
        if email.endswith(".edu"):
            br.form['q'] = email  # query with email
        else:
            br.form['q'] = first_name + ' ' + last_name  # query with name
        data = br.submit()
        soup = BeautifulSoup(data.read(), 'html.parser', from_encoding='utf-8')
        # DEGREE
        degree = find_degree_info(soup)
        count_degrees.append(degree)
        # UNIVERSITY
        university = find_university_info(soup)
        # GENDER
